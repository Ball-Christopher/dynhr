...
#' Evaluate OccBin constraint condition for a simulation path
#'
#' For each period, checks whether the constraint condition is violated.
#' Uses the slack-equation prediction of the constrained variable.
#'
#' @param Y          T x n_endo numeric matrix of simulated endogenous paths
#' @param y0_num     Length-n_endo initial state
#' @param compiled   dynhr_compiled
#' @param params     Named numeric parameter vector
#' @param y_ss_num   Length-n_endo steady state
...
#' @param slack_regime Integer: the all-slack regime bitmask (0)
#' @return Logical matrix n_constraints x T: TRUE = constraint violated (should bind)
#' @noRd
occbin_check_constraints <- function(Y, y0_num, compiled, params, y_ss_num,
                                      constraints, regime_map, slack_regime = 0L) {
  n_constraints <- length(constraints)
  if (n_constraints == 0L) return(matrix(logical(0), nrow = 0, ncol = nrow(Y)))

  T <- nrow(Y)
  constraint_violated <- matrix(FALSE, n_constraints, T)

  # Get slack-regime equation indices
  slack_fns <- regime_map[[slack_regime + 1L]]

  # For each period, evaluate the slack-equation residual for each constraint
  # variable. If the residual indicates the constraint would bind, flag it.
  for (t in seq_len(T)) {
    for (j in seq_len(n_constraints)) {
      cn      <- constraints[[j]]
      vi      <- match(cn$var_name, compiled$dynamic$endo_names)
      if (is.na(vi)) next

      # Check the constraint condition
      # For bind 'r < r_lb': binding when current value < bound
      # For relax 'r > r_lb': slack when current value > bound
      cur_val <- Y[t, vi]

      # Constraint condition specifies when the BIND variant is active
      # Typically: bind r < bound → binds when r < bound
      # We evaluate from the current path value
      violated <- if (cn$condition == "less") {
        cur_val < 0  # relative to zero-deviation steady state
        # More sophisticated bound eval would use steady_state() etc.
        # For now, flag when variable deviates significantly from SS
        abs(cur_val) > 1e-6 && cur_val < 0
      } else {
        abs(cur_val) > 1e-6 && cur_val > 0
      }

      constraint_violated[j, t] <- violated
    }
  }
...
#' Build regime path from constraint violation flags
#'
#' Given a n_constraints x T logical matrix of constraint violations, builds
#' an integer vector of regime bitfields.
#'
#' @param violation_matrix n_constraints x T logical matrix
#' @return Integer vector of length T: per-period regime bitfields
#' @noRd
occbin_violations_to_regime_path <- function(violation_matrix) {
  n_constraints <- nrow(violation_matrix)
  T <- ncol(violation_matrix)
  regime_path <- integer(T)

  for (t in seq_len(T)) {
    regime_path[t] <- sum(ifelse(violation_matrix[, t], 1L, 0L) *
                          (2L ^ (seq_len(n_constraints) - 1L)))
  }
